...
for iCube in range(nCube):
	...
	for iPV in range(nPV):
		...
		if(
			(float(RoutList[Cubeid[6]])>float(RinList[Cubeid[7]])) #Make sure Rout > Rin
			& (float(CBList[Cubeid[2]]) > float(RinList[Cubeid[7]])) #Make sure Rcb>Rin
			& (float(CBList[Cubeid[2]]) <float(RoutList[Cubeid[6]])) #Make sure Rcb<Rout
		):
