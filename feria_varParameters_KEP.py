...
#this is now setup for per-emb-30
#MAKE SURE TO COPY THE PV STUFF
...
line = 'C18O_21'
restfreq = '219.5603541' #GHz

pixSize = '0.054' #arcsec
velRes = '-0.041666785891352' #km/s


DList = ['300']
MList = ['0.1','0.3','0.5','0.7','0.9','1.1','1.3','1.5']
CBList = ['Rout']
IList = ['44']
PAList = ['77']
RotList = ['1', '-1']
...
BminList = ['0.395']
BpaList = ['-9.78']
#MAKE SURE TO COPY THIS ALSO
PVpaList = PAList	#, '80', '110', '140', '170', '200']
PVraList = ['0.']
PVdecList = ['0.']
...
for iCube in range(nCube):
	...
	for iPV in range(nPV):
		...
		if(
			(float(RoutList[Cubeid[6]])>float(RinList[Cubeid[7]])) #Just >, as if Rout=Rin there is 0 thickness
			#& (float(CBList[Cubeid[2]]) >= float(RinList[Cubeid[7]]))
			#&(float(CBList[Cubeid[2]]) <=float(RoutList[Cubeid[6]]))
		):
			makeCube(DList[Cubeid[0]], MList[Cubeid[1]], RoutList[Cubeid[6]], IList[Cubeid[3]],
			...
			pass #Don't want to make an invalid model           
